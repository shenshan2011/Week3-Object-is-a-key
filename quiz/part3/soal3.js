...
function countProfit(shoppers) {
    ...
    let result = listBarang.map(([product, price, stock]) => ({
        product,
        shoppers: [],
        leftOver: stock,
        totalProfit: 0
    ...
    for (let {name, product, amount} of shoppers) {
        let item = result.find(item => item.product === product)
        let hargabarang = listBarang.find(([p]) => p === product)
        if (item && hargabarang && item.leftOver >= amount) {
            item.shoppers.push(name);
            item.leftOver -= amount
            item.totalProfit += amount * hargabarang[1];
        }
    }
    return result;
  }
